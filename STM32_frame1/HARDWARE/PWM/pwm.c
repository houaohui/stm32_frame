...
{
    float temp_pwm,temp_duty;

    temp_pwm = steer_mid - (angle / STEER_RATIO);

    if(temp_pwm > steer_max)
    {
        temp_pwm = steer_max;
    }
    else if(temp_pwm < steer_min)
    {
        temp_pwm = steer_min;
    }

    temp_duty = temp_pwm;      //计算占空比
	switch(steer)
	{
		case 0:TIM_SetCompare2(TIM2,temp_duty);	break;
		case 1:TIM_SetCompare3(TIM2,temp_duty);	break;
		case 2:TIM_SetCompare4(TIM2,temp_duty);	break;
	}
